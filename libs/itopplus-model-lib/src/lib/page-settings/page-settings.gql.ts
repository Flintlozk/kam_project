...
export const PageSettingsTypeDefs = gql`
  ...
  extend type Query {
    togglePageSetting(status: Boolean, type: PageSettingType): Boolean
    getPageSettings: [PageSettings]
    getPageSetting(type: PageSettingType): IPageSettings
    getPageDefaultSetting(type: PageSettingType): String
    verifyWebhookURLHandler(url: String): Boolean
    getWebhookPatternList: [ResponseWebhookPatternList]
  }
  extend type Mutation {
    saveClosedCustomerWebhookURL: Boolean
    saveWebhookURL(url: String, type: PageSettingType): Boolean
    setWorkingHourSetting(config: PageWorkingHoursOptionsInput): Boolean
    ...
    setMessageTrackMode(config: PageMessageTrackModeOptionsInput): Boolean
