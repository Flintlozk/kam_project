...
@Directive({
  selector: '[reactorRoomCustomerLineChart]',
})
export class CustomerLineChartDirective implements OnChanges {
  @Input() linechartData;
  ...
  ngOnChanges(e): void {
    if (e?.linechartData?.currentValue?.length && this.linechartData.length) {
      ...
      this.linechartLabel = this.linechartData?.map((item) => item[this.linechartXKey]);
