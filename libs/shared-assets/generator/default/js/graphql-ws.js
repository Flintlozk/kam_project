    ...
    switch (val.type) {
      ...
      case MessageType.Subscribe:
        return (
          ...
          (!hasOwnProperty(val.payload, 'variables') || val.payload.variables === undefined || val.payload.variables === null || hasOwnObjectProperty(val.payload, 'variables')) &&
          (!hasOwnProperty(val.payload, 'extensions') || val.payload.extensions === undefined || val.payload.extensions === null || hasOwnObjectProperty(val.payload, 'extensions'))
        );
...
function createClient(options) {
  ...
  async function connect() {
    const [socket, throwOnClose] = await (connecting !== null && connecting !== void 0
      ? connecting
      : (connecting = new Promise((connected, denied) =>
          (async () => {
            if (retrying) {
...
export { CloseCode, GRAPHQL_TRANSPORT_WS_PROTOCOL, MessageType, createClient, isMessage, parseMessage, stringifyMessage };
