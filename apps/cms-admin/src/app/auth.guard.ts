...
export class AuthGuard implements CanActivate {
  ...
  expiredIn: number;
  constructor(private router: Router, private apollo: Apollo, private ngZone: NgZone, private authService: AuthService) {}

  checkAuth(): Observable<boolean> {
    return this.verifyAdminCMSAuth().pipe(
  ...
  verifyAdminCMSAuth(): Observable<IHTTPResult> {
    return this.apollo
      .query({
        query: gql`
          query verifyAdminCMSAuth {
            verifyAdminCMSAuth {
              status
              value
            }
      ...
      .pipe(
        map((x) => x.data['verifyAdminCMSAuth']),
