...
class Product {
  ...
  async getProductByIDHandler(parent, args: IArgsProductID, context: IGQLContext): Promise<IProductByID[]> {
    ...
    const { id } = productData;
    const { pageID } = context.payload;
    const data = await Product.productService.getProductByID(pageID, id, context.payload.subscriptionID);
    return data;
  }
  ...
  async updateProductVariantsHandler(parent, args, context: IGQLContext) {
    ...
    const data = await Product.productService.updateProductVariants(pageID, id, productData, context.payload.subscriptionID);
    return data;
  }
