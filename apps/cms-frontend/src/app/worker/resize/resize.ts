import initResizeWasm, { resize as wasmResize } from '../../../../../wasm-codecs/resize/pkg';
import initHqxWasm, { resize as wasmHqx } from '../../../../../wasm-codecs/hqx/pkg';
...
function clamp(num: number, { min = Number.MIN_VALUE, max = Number.MAX_VALUE }: ClampOpts): number {
...
async function hqx(input: ImageData, opts: HqxResizeOptions): Promise<ImageData> {
  if (!hqxWasmReady) {
...
export default async function resize(data: ImageData, opts: WorkerResizeOptions): Promise<ImageData> {
  let input = data;
