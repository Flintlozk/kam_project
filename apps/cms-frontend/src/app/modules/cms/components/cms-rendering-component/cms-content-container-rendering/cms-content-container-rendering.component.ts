...
import { ComponentType, DragRefData, Dropped, DropZoneData, EDropzoneType } from '../../../modules/cms-edit-mode/components/cms-edit-rendering/cms-edit-rendering.model';
...
import { CmsContentManagementLandingRenderingComponent } from '../cms-content-management-landing-rendering/cms-content-management-landing-rendering.component';
...
import { CONTENT_CONTAINER } from './token';

@Component({
  selector: 'cms-next-cms-content-container-rendering',
  templateUrl: './cms-content-container-rendering.component.html',
  styleUrls: ['./cms-content-container-rendering.component.scss'],
  providers: [
    { provide: Children, useExisting: forwardRef(() => CmsContentContainerRenderingComponent) },
    { provide: CONTENT_CONTAINER, useExisting: forwardRef(() => CmsContentContainerRenderingComponent) },
  ],
})
export class CmsContentContainerRenderingComponent
  extends Mixin(ComponentCommonDirective, ComponentDesignDirective, ComponentSettingDirective)
  implements OnInit, AfterViewInit, AfterContentInit, OnDestroy
{
  @ViewChild('contentContainerDropZone', { static: true }) contentContainerDropZone: ElementRef<HTMLElement>;
  @ViewChild('contentContainerInsertPoint', { static: true, read: ViewContainerRef }) contentContainerInsertPoint: ViewContainerRef;
  @ContentChildren(Children) public cmsComponentRenderingChildren!: QueryList<
  ...
  ngAfterContentInit(): void {
    this.cmsComponentRenderingChildren.forEach((c, i) => {
      ...
      this.contentContainerInsertPoint.move(rootViewRef, i);
    });
    this.cmsEditService.setContentInsertPointContainer(this.contentContainerInsertPoint);
  }

  ngAfterViewInit(): void {
    this.createDropZoneListRef();
    this.cmsEditService.dropZoneDropListRef.dropped.pipe(takeUntil(this.destroy$)).subscribe((event) => {
      const {
      ...
      const dropZoneViewRefAndElementRefAndComponents = this.cmsEditService.getDropZoneViewRefAndElementRefAndComponents();
      ...
      if (dropZonePreviousContainer === dropZoneContainer) {
        ...
        this.moveViewRefTo(this.contentContainerInsertPoint, component.viewRef, dropZoneCurrentIndex);
      ...
      } else if (dropZonePreviousContainer === this.cmsEditService.menuDropListRef && dropZoneContainer === this.cmsEditService.dropZoneDropListRef) {
        // MENU TO DROP ZONE
        ...
        if (genericType === MenuGenericType.TEMPLATE_ELEMENTS) {
          createdComponent = this.checkThemeElement(type, this.contentContainerInsertPoint, dropZoneCurrentIndex, dropZoneItem);
        } else if (genericType === MenuGenericType.LAYOUT) {
          createdComponent = this.checkLayoutElement(type, this.contentContainerInsertPoint, dropZoneCurrentIndex);
          event.item.data.component = createdComponent;
        } else {
          createdComponent = this.menuToDropZoneInitComponent(type, genericType, this.contentContainerInsertPoint, dropZoneCurrentIndex, dropZoneDropListRef);
        }
        ...
        this.moveViewRefTo(this.contentContainerInsertPoint, component.viewRef, dropZoneCurrentIndex);
        this.moveLayoutData(dropZonePreviousContainer, this.cmsEditService.dropZoneDropListRef, i, dropZoneCurrentIndex);
        ...
        this.moveViewRefTo(this.contentContainerInsertPoint, component.viewRef, dropZoneCurrentIndex);
        this.moveLayoutData(dropZonePreviousContainer, this.cmsEditService.dropZoneDropListRef, from, dropZoneCurrentIndex);
        const dragRef = this.createAndInsertDragRefToContainer(
          component.el,
          dropZoneCurrentIndex,
          this.cmsEditService.dropZoneDropListRef,
  ...
  createDropZoneListRef(): void {
    const dropListRef = this.dragDrop.createDropList<DropZoneData>(this.contentContainerDropZone).withItems([]);
    dropListRef.sortingDisabled = false;
    dropListRef.data = { dragRefs: [], viewRefAndElementRefAndComponents: [], dropzoneType: EDropzoneType.CONTENT };
    this.cmsEditService.addDropListRef(dropListRef);
  }
  ...
  menuToDropZoneInitComponent(type: MenuType, genericType: MenuGenericType, vcr: ViewContainerRef, to: number, dropListRef: DropListRef<DropZoneData>): ComponentType {
    switch (genericType) {
      ...
      case MenuGenericType.SHOPPING_CART:
        return this.checkShoppingCartElement(type as ShoppingCartTypes, vcr, to);
      default:
        break;
    }
  }
