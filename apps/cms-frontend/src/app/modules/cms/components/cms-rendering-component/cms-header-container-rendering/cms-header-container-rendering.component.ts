...
import { getRootViewRef } from 'apps/cms-frontend/src/app/shares/utils';
import {
  ButtonType,
  EDropzoneType,
  EFontFamilyCode,
...
import { Children } from 'apps/cms-frontend/src/app/shares/utils';
...
import { HEADER_CONTAINER } from './token';
import { CmsCommonService } from '../../../services/cms-common.service';
@Component({
  selector: 'cms-next-cms-header-container-rendering',
  templateUrl: './cms-header-container-rendering.component.html',
  styleUrls: ['./cms-header-container-rendering.component.scss'],
  providers: [
    { provide: Children, useExisting: forwardRef(() => CmsHeaderContainerRenderingComponent) },
    { provide: HEADER_CONTAINER, useExisting: forwardRef(() => CmsHeaderContainerRenderingComponent) },
  ],
})
export class CmsHeaderContainerRenderingComponent
...
{
  @ViewChild('headerContainerDropZone', { static: true }) headerContainerDropZone: ElementRef<HTMLElement>;
  @ViewChild('headerContainerInsertPoint', { static: true, read: ViewContainerRef }) headerContainerInsertPoint: ViewContainerRef;
  ...
  ngAfterContentInit(): void {
    this.cmsComponentRenderingChildren.forEach((c, i) => {
      ...
      this.headerContainerInsertPoint.move(rootViewRef, i);
    });
    this.cmsEditService.setHeaderInsertPointContainer(this.headerContainerInsertPoint);
  }
  ngOnDestroy(): void {
  ...
  ngAfterViewInit(): void {
    ...
    this.cmsEditService.headerDropListRef.dropped.pipe(takeUntil(this.destroy$)).subscribe((event) => {
      const {
      ...
      const dropZoneViewRefAndElementRefAndComponents = this.cmsEditService.getHeaderDropZoneViewRefAndElementRefAndComponents();
      ...
      if (dropZonePreviousContainer === dropZoneContainer) {
        ...
        this.moveViewRefTo(this.headerContainerInsertPoint, component.viewRef, dropZoneCurrentIndex);
      ...
      } else if (dropZonePreviousContainer === this.cmsEditService.menuDropListRef && dropZoneContainer === this.cmsEditService.headerDropListRef) {
        // MENU TO DROP ZONE
        ...
        if (genericType === MenuGenericType.TEMPLATE_ELEMENTS) {
          createdComponent = this.checkThemeElement(type, this.headerContainerInsertPoint, dropZoneCurrentIndex, dropZoneItem);
        } else if (genericType === MenuGenericType.LAYOUT) {
          createdComponent = this.checkLayoutElement(type, this.headerContainerInsertPoint, dropZoneCurrentIndex);
          event.item.data.component = createdComponent;
        } else {
          createdComponent = this.menuToDropZoneInitComponent(type, genericType, this.headerContainerInsertPoint, dropZoneCurrentIndex, dropZoneDropListRef);
        }
        ...
        this.moveViewRefTo(this.headerContainerInsertPoint, component.viewRef, dropZoneCurrentIndex);
        this.moveLayoutData(dropZonePreviousContainer, this.cmsEditService.headerDropListRef, i, dropZoneCurrentIndex);
        ...
        this.moveViewRefTo(this.headerContainerInsertPoint, component.viewRef, dropZoneCurrentIndex);
        this.moveLayoutData(dropZonePreviousContainer, this.cmsEditService.headerDropListRef, from, dropZoneCurrentIndex);
        const dragRef = this.createAndInsertDragRefToContainer(
          component.el,
          dropZoneCurrentIndex,
          this.cmsEditService.headerDropListRef,
  ...
  createDropZoneListRef(): void {
    const dropListRef = this.dragDrop.createDropList<DropZoneData>(this.headerContainerDropZone).withItems([]);
    dropListRef.sortingDisabled = false;
    dropListRef.data = { dragRefs: [], viewRefAndElementRefAndComponents: [], dropzoneType: EDropzoneType.THEME_HEADER };
    this.cmsEditService.addHeaderDropListRef(dropListRef);
  }
